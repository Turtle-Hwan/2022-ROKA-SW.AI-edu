...
---
# 07
# 2주차 실전 프로젝트 - 빙고 게임 만들기
## 구현에 앞서
온라인 빙고 게임
여기서 만들고자 하는 빙고 게임은 우리가 흔히 5 X 5 칸을 그린 후 번갈아 가면서 숫자를 부르고, 해당 숫자들을 지워 가로나 세로, 대각선으로 먼저 빙고를 만드는 사람이 이기는 바로 그 게임입니다. 숫자가 아닌 음식, 연예인 이름 등으로 바꿔서 하기도 하죠. 모두 학창 시절 한 번쯤은 이 게임을 해보았을 거라 생각합니다.

원래 빙고 게임은 자신이 원하는 칸에 자신이 원하는 숫자나 내용을 적고 시작하는 게임이지만 여기에서는 이러한 입력이 중요한 것은 아니므로 무작위로 숫자가 입력되도록 하였으며, 웹 소켓에 익숙해지는 것에 중점을 두고자 게임의 승부를 판가름 짓는 요소는 구현하지 않았습니다. 또한 전 챕터의 채팅 페이지처럼 유저 이름을 입력받아서 설정하게 할 수도 있지만, 여기서는 유저 이름을 url에 바로 입력받도록 하였습니다. 강좌를 따라 빙고 게임을 다 만들고 나면 추가기능 구현에도 도전해보세요

주요 기능
이번 챕터에서 구현할 빙고 게임에는 아래와 같은 주요 기능이 있습니다.
...
## 빙고 게임 - 코드
먼저 어떤 식으로 페이지가 만들어져 있는지 보도록 하겠습니다. 오른쪽 실습화면에서 실행해보세요. 맨 처음 접속 url에 들어가면 username이 입력되어 있지 않기 때문에 아래와 같이 아무런 이름도 뜨지 않습니다.

그 상태에서 url 맨 끝 부분에 &username=test1 과 같이 입력하면 username을 test1로 설정하고, 이름이 제대로 뜨게 됩니다. 아래 화면은 test1과 test2로 접속한 후 게임을 시작한 화면입니다.
(구름EDU 페이지 내에서 실행되지 않을 경우 url 오른쪽의 새 창 열기 버튼을 클릭해주세요.)


이제 코드를 작성해보도록 하겠습니다. 앞서 채팅을 만들 때와 같이 express-generator를 이용하여 새 프로젝트 폴더를 하나 생성해줍니다.

그리고 채팅과 마찬가지로 pug, socket.io를 설치해줍니다. 이미 socket.io를 글로벌로 설치해주신 분들은 채팅 때와 마찬가지로 npm link socket.io로 위치를 잡아주세요.

서버를 돌려주는 server.js를 작성해봅시다. server.js 에는 서버를 구동시킬 수 있는 소스가 들어있습니다. 이번에도 채팅과 마찬가지로 server.js 하나에 서버 코드를 모두 작성하도록 하겠습니다.
```js
// server.js
...
이제 화면을 보여주는 layout.pug와 main.pug를 작성해보도록 하겠습니다. 이 파일들은 public/views 폴더에 생성하시면 됩니다. 

화면을 꾸며주는 CSS 파일도 생성해봅시다. 채팅 때 만들어 보았으니 알고 있겠지만,
public/stylesheets에 생성해주시면 됩니다.

클라이언트 쪽 이벤트를 처리해주는 main.js 입니다. 채팅 페이지를 만들 때에는 chat.pug에 모두 작성했지만, 이번 빙고 게임은 채팅보다 처리해줘야 할 이벤트들이 많기 때문에 main.js로 따로 분리했습니다.



## 빙고 게임 - 설명
게임화면을 구성하는 main.pug 코드는 간단하게 설명하도록 하겠습니다. table로 숫자를 출력하는 표를 만들어주었고, 사용자를 출력하는 list와 알림 문구를 출력하는 log를 넣어주었습니다. 또한 url에서 받아온 사용자 이름은 클라이언트 쪽에서 계속해서 쓰이기 때문에, username이라는 input에 값을 넣어준 후 보이지 않게 hidden 처리를 해 주었습니다.

먼저 서버 쪽 처리를 해주는 server.js 코드를 보도록 하겠습니다.
...
```
제일 먼저 소켓이 연결 됐을 때 발생하는 이벤트인 connection 입니다. 서버 쪽에서 처리하는 이벤트들은 모두 connection 안에 넣어주었습니다.

connection 밖에는 전역 변수로 계속 쓰이는 users, user_count, turn_count 변수를 선언해주었습니다. 사용자를 저장하기 위한 객체와 현재 접속한 사용자의 수, 그리고 누구의 차례인지 기록하는 변수입니다. 

먼저 join 이벤트부터 살펴보겠습니다. 사용자가 접속했을 때 발생하는 join 이벤트입니다. 나중에 main.js를 보면 아시겠지만, 클라이언트 페이지에서 connect 이벤트가 발생하면 join으로 데이터를 넘겨줍니다. 사용자의 이름을 가져와서 소켓의 사용자 이름으로 설정하고 사용자 객체를 생성하여 이름과 턴 값을 설정합니다. 그리고 클라이언트에게 새로운 사용자가 접속했으니 사용자 목록을 업데이트하여 출력하라고 update_users라는 이벤트를 전송하여 알려줍니다.

```js
// server.js - game_start
...
```
클라이언트 쪽에서 게임 시작 버튼을 누르면 발생하는 이벤트인 game_start 입니다. game_started 이벤트를 발생시켜 모든 사용자에게 게임 시작을 알리는 이벤트입니다. 또한 현재 순서에 따라 해당 사용자의 턴 값을 true로 바꾸어 해당 사용자가 빙고판의 숫자를 선택할 수 있게 합니다. 그 후 해당 사용자의 순서를 update_users라는 이벤트를 전송하여 알려줍니다. 
...
```
숫자를 선택할 때 발생하는 이벤트인 select 입니다. 현재 사용자의 턴을 종료시키고 다음 사용자의 턴을 true 값으로 바꾸도록 합니다. 마찬가지로 다음 사용자의 순서를 알려주기 위해 이번에도 update_users라는 이벤트를 전송합니다.
...
```
마지막으로 사용자가 접속을 종료했을 때 발생하는 disconnect 이벤트입니다. users 배열에서 현재 소켓의 아이디와 같은 값을 가진 사용자를 삭제하고, user_count를 감소시킵니다. 그리고 사용자가 나가면 상대방도 알 수 있도록 유저 리스트를 업데이트합니다.


#### main.js
```js
// main.js - init의 socket 이벤트들
...
```
클라이언트 쪽 처리를 해주는 main.js를 살펴봅시다.

bingo라는 객체를 통해 다양한 동작을 수행할 수 있도록 구성하였으며, html 로딩이 끝나면 bingo.init()으로 bingo 객체를 초기화하면서 빙고 게임이 설정되도록 하였습니다. 

코드를 살펴보면 크게 6가지 메소드로 이루어져 있습니다.

첫 번째 메소드는 초기화를 담당하는 메소드 init()인데, 이것이 사실 가장 중요하다고 할 수 있습니다. 소켓에 이벤트별로 콜백 함수를 할당하는 부분이기 때문입니다. init() 메소드를 조금 더 자세히 살펴보면, 서버로부터 check_number, game_started, update_users, connect와 같은 이벤트에 맞게 알맞은 콜백 함수를 할당하는 것을 확인할 수 있습니다.


```js
// main.js - init의 클릭 이벤트들
...
```
그리고 나머지 내용을 조금 더 살펴보면, 빙고판을 랜덤하게 생성하는 코드와 빙고판을 클릭할 때 해당 숫자를 서버에 전송하여 다른 사용자에게 전달될 수 있도록 하는 부분이 이벤트 콜백 함수로 지정되어 있습니다. 그리고 게임 시작 버튼에 대한 이벤트 콜백 함수도 할당되어 있습니다. 숫자를 클릭할 때나 시작 버튼을 클릭할 때에 혼자서는 게임을 실행할 수 없도록 user_cnt 값을 체크하여 알림을 주도록 하였습니다. 이 user_cnt는 update_users가 실행될 때마다 업데이트됩니다.
...
```
select_num은 사용자가 숫자를 클릭했을 때 사용되는 함수로써, 자신의 차례일 때는 select 이벤트를 발생시킵니다. 그리고 turn을 false로 바꾸어 줍니다. 만약 자신의 차례가 아닐 경우에는 알림을 출력합니다.


```js
// main.js - where_is_it, check_num
...
```
다음으로는 where_is_it()이라는 메소드인데, 이 메소드 역할은 단순합니다. 바로 상대방이 선택한 숫자가 어디인지 찾아서 check_num()이라는 메소드를 호출하는 것입니다. 이어서 check_num()이라는 메소드를 확인해보면 상대방이 선택한 숫자를 선택할 수 없는 상태로 만들고 화면에 표시해주는 역할을 합니다.


```js
// main.js - update_userlist
...
```
마지막으로 update_userlist()와 print_msg()라는 메소드는 각각 사용자의 목록과 순서를 표시해주고 사용자가 어떤 숫자를 선택했는지 등을 표시해주는 메세지를 출력하는 함수들입니다. 자신의 이름이 출력될 때에는(소켓 아이디를 비교해서 판단) 색깔을 다르게 주어 편의성을 높였고, 문구가 출력될 때에도 스크롤이 가장 아래로 내려가게 해주었습니다. 


## 빙고 게임 - 도전 문제
빙고 프로젝트가 완성되었습니다. 

하지만 처음에 언급했듯이, 빙고가 완성되었을 때 체크하는 기능이나 이름을 입력받는 기능 등은 구현하지 않았습니다. 채팅과 빙고 프로젝트를 차근차근 따라오신 분들은 충분히 도전해 보실만한 기능입니다. 추가 기능을 구현해보면 실력향상에도 많은 도움이 됩니다. 

빙고 게임 프로젝트에 더 다양한 기능을 추가하는데 도전해보세요.

1. 부트스트랩을 이용해 UI 더 예쁘게 꾸며보기
2. 빙고가 완성되었을 때 실시간으로 체크해서 승부를 결정하기
